...
export function TryPageClient({ font }: Props) {
  ...
  const specimenStyle: React.CSSProperties = {
    fontFamily: font.fontFamily ?? "var(--font-display), Georgia, serif", // ← fix
    ...
    fontVariantLigatures: activeFeatures.has("liga")
      ? "common-ligatures"
      : "none",
  };

  const groupedFeatures = OPENTYPE_FEATURES.filter(
    (f) => f.group === activeGroup,
  );
      ...
      <div className="flex flex-col lg:flex-row min-h-[calc(100vh-112px)]">
        ...
        >
          ...
          >
            <button
              onClick={() =>
                setActiveFeatures(new Set(OPENTYPE_FEATURES.map((f) => f.id)))
              }
            ...
            <button
              onClick={() =>
                setActiveFeatures(new Set(["liga", "kern", "calt"]))
              }
              className={`flex-1 text-[8px] tracking-[0.15em] uppercase py-2 border transition-all ${darkMode ? "border-white/15 text-white/40 hover:border-white/40 hover:text-white" : "border-black/15 text-muted hover:border-ink hover:text-ink"}`}
            >
              Reset
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          ...
          <AnimatePresence>
            {previewFeature &&
              (() => {
                const feat = OPENTYPE_FEATURES.find(
                  (f) => f.id === previewFeature,
                )!;
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-px">
                      {samples.map((s, i) => (
                        ...
                        >
                          ...
                          >
                            Feature {activeFeatures.has(feat.id) ? "ON" : "OFF"}{" "}
                            → ON
                          </p>
          ...
          >
            <div>
              ...
              >
                {font.styles} styles · {font.category} · Desktop, Web & App
                licenses available
              </p>
            </div>
